...
def save_to_csv(filelist, labels, filename):
    data = {'file': filelist, 'label': labels}
...
def get_train_test_data(ham_path, spam_path, enron_indices, min_doc_count, binary=False):
    ...
    # Save raw data and labels to CSV before transformation
    save_to_csv(filelist, labels, 'raw_data.csv')
...
if __name__ == '__main__':
    ...
    print('{} training samples, {} test samples, {} total'.format(train_data[0].shape[0], test_data[0].shape[0], train_data[0].shape[0] + test_data[0].shape[0]))
    print('Number of features: {}'.format(train_data[0].shape[1]))
    quick_test(train_data, test_data)
